...
public interface ClientRequestContext extends RequestContext {
    ...
    /**
     * Sets a handler to run when the response times out. {@code responseTimeoutHandler} must abort the request,
     * e.g., by calling {@link HttpRequest#abort(Throwable)}. If not set, the request will be aborted with
     * {@link TimeoutException}.
     *
     * <p>For example,
     * <pre>{@code
     *   HttpRequest req = HttpRequest.streaming();
     *   ctx.setResponseTimeoutHandler(() -> {
     *      req.abort(new IllegalStateException("Server is in a bad state."));
     *   });
     *   ...
     * }</pre>
     */
