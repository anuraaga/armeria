...
final class Utils {
  ...
  static Type getFutureReturnType(Type returnType) {
    if (!(returnType instanceof ParameterizedType)) {
      throw new IllegalArgumentException(
          "Call return type must be parameterized as Future<Foo> or Future<? extends Foo>");
    }
    final Type responseType = getParameterUpperBound(0, (ParameterizedType) returnType);

    // Ensure the Call response type is not Response, we automatically deliver the Response object.
    if (HttpResponse.class.isAssignableFrom(getRawType(responseType))) {
      throw new IllegalArgumentException(
          "Future<T> cannot use HttpResponse as its generic parameter. "
